...
%% feed input
callmxnetLite('MXPredSetInput', pCtx, 'data', inImg, lenImg);

%% forward
callmxnetLite('MXPredForward', pCtx);
...
callmxnetLite('MXPredGetOutputShape', ...
    pCtx, ...
    index, ...
    out_shape, ...
    out_dim);
...
%% Free the model
callmxnetLite('MXPredFree', pCtx);
model.predictor = libpointer('voidPtr', 0);
